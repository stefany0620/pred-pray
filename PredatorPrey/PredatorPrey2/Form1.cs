...
namespace PredatorPrey
{
    public partial class MainForm : Form
    {
        ...
        public void Reset()
        {
            ...
            wallsArea = new int[width, height];
        ...
        private void DrawWalls(Graphics g)
        {
            for (int x = 0; x < width; x++)
                for (int y = 0; y < height; y++)
                    if (wallsArea[x, y] == 1)
                        g.FillRectangle(new SolidBrush(Color.DimGray), x * cellSize, y * cellSize, cellSize, cellSize);
        }
        ...
        {
            return wallsArea[x, y] == 0 && (particlesArea[x, y] == 0 || particlesArea[x, y] == (int)particleType);
        }
        ...
        void GenerateInsideWalls(int x, int y, float probability = 0.95f)
        {
            ...
            float targetProb = (float) Math.Pow(probability, 2);
            GenerateInsideWalls(x, y - 1, targetProb);
            GenerateInsideWalls(x, y + 1, targetProb);
            GenerateInsideWalls(x - 1, y, targetProb);
            GenerateInsideWalls(x + 1, y, targetProb);
        }

        void GenerateEdgesWalls()
        { 
            int maxWallsSize = (int)Math.Round(Math.Sqrt(width * height) * .1f);

            // Generate up walls
            int currentWallSize = random.Next(maxWallsSize);
            int scaler = 1;

            Vector2[] turns = new Vector2[4] { Vector2.Up, Vector2.Right, Vector2.Down, Vector2.Left };

            foreach (Vector2 turn in turns)
            {
                for (int w = 0; w < width; w++)
                {
                    for (int s = 0; s < currentWallSize; s++)
                    {
                        if (turn == Vector2.Up)
                            wallsArea[w, s] = 1;
                        if (turn == Vector2.Down)
                            wallsArea[w, height - s - 1] = 1;
                        if (turn == Vector2.Left)
                            wallsArea[s, w] = 1;
                        if (turn == Vector2.Right)
                            wallsArea[width - s - 1, w] = 1;
                    }

                    if (currentWallSize >= maxWallsSize || currentWallSize < 0 || random.NextDouble() > .3f)
                        scaler *= -1;

                    currentWallSize += random.Next(0, 2) * scaler;
                }
            }

            int insideWallsCount = random.Next(maxWallsSize, maxWallsSize * 2);
            for (; insideWallsCount > 0; insideWallsCount--)
            {
                int x = random.Next(width);
                int y = random.Next(height);
                GenerateInsideWalls(x, y);
            }
        }
